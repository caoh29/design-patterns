...
class Pokemon {
  ...
  clone(): Pokemon {
    ...
    return new Pokemon(this.name, this.type, this.level, Array.from(this.attacks));
  }
...
function mainProto() {
  const charmander = new Pokemon("Charmander", "Fuego", 1, ["Llamarada", "Arañazo", "Cabeza caliente"]);

  const charmander_hijo = charmander.clone();
  charmander_hijo.name = "Charmeleon";
  charmander_hijo.level = 16;
  charmander_hijo.attacks.push("Lanzallamas");

  charmander.displayInfo();
  console.log("--------------------");
  charmander_hijo.displayInfo();
}
